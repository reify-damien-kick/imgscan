...
The `home-routes` handler in the `imgscan.routes.home` namespace
...
The `home-page` function will in turn call the `imgscan.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `imgscan.handler` namespace:
...
Request middleware functions are located under the `imgscan.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `imgscan.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `imgscan.db.core` in some other namespace.
